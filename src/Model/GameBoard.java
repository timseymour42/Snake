...
//  }
  ...
  public void updateFood() {
    // where food cannot be
    List<Cell> cellsToAvoid = this.snake.getSnakeCoords();
    cellsToAvoid.addAll(this.food.getFoodCoords());

    int randRow = (int) Math.round(Math.random() * (this.height - 1));
    int randCol = (int) Math.round(Math.random() * (this.width - 1));
    if (cellsToAvoid.contains(new Cell(randRow, randCol))) {
      this.updateFood();
    }
    else {
      this.food.getFoodCoords().add(new Cell(randRow, randCol));
